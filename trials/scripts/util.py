...
def get_metrics(
    ...
    sharpe_ratio = float(
        stats.sharpe_ratio(noncumulative_returns, risk_free)
    )
    if math.isinf(sharpe_ratio):
        sharpe_ratio = 0
    annual_return = float(stats.annual_return(noncumulative_returns))
    annual_volatility = float(
        stats.annual_volatility(noncumulative_returns)
    )
    max_drawdown = float(stats.max_drawdown(noncumulative_returns))
