...
from trials.empyrical import stats
...
class ReinforceTradingEnv(TradingEnv):
    ...
    def reset(self, *, seed=None, options=None):
        super().reset(seed=seed)  # importante!

        self.euclidean_dist = float("inf")

        # se houver estado interno, reinicialize aqui
        # ex:
        # self.current_step = 0
...
class StepTradingEnv(gym.Env):
    def __init__(
        ...
        observation_dict = {
            ...
            "action": gym.spaces.Box(-1, 2, shape=(max_len,), dtype=int),
            "mask_len": gym.spaces.Box(0, max_len, shape=(1,), dtype=int),
        }
    ...
    def get_curr_sharpe_ratio(self):
        ...
        sharpe_ratio = float(
            stats.sharpe_ratio(noncumulative_returns, self.risk_free)
        )
